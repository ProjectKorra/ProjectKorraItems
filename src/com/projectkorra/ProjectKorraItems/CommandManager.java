			...
			public boolean onCommand(CommandSender s, Command c, String label, String[] args) {
				...
				/* GIVING */
					...
					if (!s.hasPermission("bendingitems.command.give")) {
						...
						s.sendMessage(ChatColor.YELLOW + " ---- " + ChatColor.GOLD + "Item Names "
								+ ChatColor.YELLOW + "----");
					...
					if (args.length == 2) {
						...
						if (player == null) {
							...
							try {
								...
								// If the qty wasn't parsable then they were
								// attemping to send a name
					...
					if (!s.hasPermission("bendingitems.command.stats")) {
						...
						for (int i = pageIndex; i < pageIndex + Messages.LINES_PER_PAGE; i++) {
							if (i >= 0 && i < attribs.size()) {
								Attribute att = attribs.get(i);
								s.sendMessage(ChatColor.YELLOW + att.getName() + ": "
										+ ChatColor.WHITE + att.getDesc());
							} else
								break;
						}
						if (phrase != null)
