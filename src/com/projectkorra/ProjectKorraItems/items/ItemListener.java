...
public class ItemListener implements Listener {
	...
	public void onCraftingPlace(InventoryClickEvent event) {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				ArrayList<ItemStack> invItemsClone = (ArrayList<ItemStack>) originalInvItems
						.clone();
				...
				for (CustomItem citem : CustomItem.items.values()) {
					...
					ArrayList<RecipeIngredient> recipeClone = (ArrayList<RecipeIngredient>) citem
							.getRecipe().clone();
					...
					for (int i = 0; i < recipeClone.size(); i++) {
						...
						for (int j = 0; j < invItemsClone.size(); j++) {
							...
							if (ing.getMaterial() == Material.AIR && item.getType() == Material.AIR) {
								...
								j--;
								break;
							} else if (ing.getMaterial() == item.getType()
									&& item.getAmount() >= ing.getQuantity()
	...
	public void onGrabResultItem(InventoryClickEvent event) {
		...
		ArrayList<RecipeIngredient> recipeClone = (ArrayList<RecipeIngredient>) citem.getRecipe()
				.clone();
		...
		while (recipeClone.size() > 0) {
			...
			if (ing.getMaterial() != Material.AIR)
				for (int i = 0; i < invItems.size(); i++) {
					...
					if (istack.getType() == ing.getMaterial()
							&& istack.getDurability() == ing.getDamage()) {
