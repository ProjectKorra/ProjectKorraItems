...
/**
 * Grappling Hooks let the player grab onto blocks and fling
 * themselves toward a destination location. They can cause a fling
 * to occur by left clicking, or they can hold sneak to slowly climb
 * the Grappling Hook, causing them to slowly ascend to the destination.
 * 
 * TODO: This class is unfinished due to some issues with making the
 * particle animations realistic for a grappling hook.
 */
public class GrapplingHook {
	...
	/**
	 * Attempt to create a new Grappling Hook, fling
	 * a currently existing grappling hook, or retract a
	 * hook, all depending on the type of action.
	...
	/**
	 * The GrapplingHookTask is the manipulates the
	 * specific instance of a GrapplingHook after it has been initially created.
	 * This task does its action based on the State of the GrapplingHook that
	 * created this task. If that variable is changed then this task will
	 * follow in turn.
	 */
	public class GrapplingHookTask extends BukkitRunnable {
		...
		public void run() {
			...
			if(state == State.START) {
				...
				/*
				 *  Use an iterator in case the speed value is too big, 
				 *  it would normally cause blocks to be skipped 
				 *  if we just jumped directly.
				 */
	...
	public static void playAnimation(Location loc1, Location loc2, double incr, float density, float r, float g, float b) {
		...
		Vector dir = Methods.getDirection(loc1, loc2).normalize();
